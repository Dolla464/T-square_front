...
function AdminGroups() {
  ...
  const { groups, pagination: apiPagination, loading, availableStudents, getGroups, getGroupById, createGroup, updateGroup, deleteGroup, getAvailableStudents, bulkAssign } = useGroups();
  ...
  // فلترة محلية تحسباً لعدم دعم الـ API للفلاتر (Local Filtering)
  const filteredGroups = useMemo(() => {
    if (!groups) return [];
    return groups.filter((group) => {
      const matchSearch =
        !debouncedSearch ||
        group.group_name?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        group.course_title?.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        group.instructor_name?.toLowerCase().includes(debouncedSearch.toLowerCase());

      const matchCourse = courseFilter === "all" || String(group.course_id) === String(courseFilter);
      const matchInstructor = instructorFilter === "all" || String(group.instructor_id) === String(instructorFilter);
      ...
      return matchSearch && matchCourse && matchInstructor && matchTime;
    });
  }, [groups, debouncedSearch, courseFilter, instructorFilter, timeFilter]);
  ...
  // اختيار/إلغاء اختيار كل الطلاب
  const toggleSelectAllStudents = () => {
    if (selectedStudents.length === availableStudents.length && availableStudents.length > 0) {
      setSelectedStudents([]);
    } else {
  ...
  // تعيين الطلاب المحددين للمجموعة
  const handleBulkAssignSubmit = async () => {
    if (selectedStudents.length === 0) return;
    try {
      await bulkAssign(editingItem, {
        student_ids: selectedStudents,
        course_id: formData.course_id
      });
      setSelectedStudents([]);
      await getAvailableStudents(editingItem);
      // تحديث الجدول ليعكس التغييرات في عدد الطلاب
      getGroups({
        page: currentPage,
        search: debouncedSearch,
        time: timeFilter === "all" ? "" : timeFilter,
      ...
      // الأخطاء يتم معالجتها في الـ hook
    }
  };
        ...
        <div className="ac-form-container">
          ...
          <div className="ac-form-body p-4 bg-white border rounded-4 shadow-sm">
            <div className="ac-tab-content basic-info">
              ...
              {/* قائمة الطلاب المتاحين للإضافة (تظهر في وضع التعديل) */}
                  ...
                  <div className="ac-table-container">
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          <h5
                            ...
                            style={{ letterSpacing: "-0.5px" }}
                          >
                            {isArabic ? "الطلاب المتاحين للإضافة" : "Available Students"}
                          </h5>
                          <p className="text-muted small mb-0">
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <div className="form-check">
                        ...
                        />
                        <label className="form-check-label ms-2 fw-medium text-dark" htmlFor="selectAllStudents" style={{ cursor: 'pointer' }}>
                      ...
                      {selectedStudents.length > 0 && (
                        <button
                          type="button"
                          className="btn btn-danger px-4 py-2 fw-medium rounded-3 shadow-sm d-flex align-items-center"
                          onClick={handleBulkAssignSubmit}
                        >
                          <i className="bi bi-check2-circle me-2 fs-5"></i>
                          {isArabic ? `تعيين (${selectedStudents.length}) طالب` : `Assign (${selectedStudents.length}) Students`}
                        </button>
                      )}
                    </div>
                    ...
                    >
                      <div className="table-responsive">
                        <table className="table mb-0 align-middle table-hover">
                          <thead>
                            <tr>
                              <th className="ps-4 py-3 border-0 text-secondary text-uppercase small fw-bold" style={{ width: "50px" }}></th>
                              <th className="py-3 border-0 text-secondary text-uppercase small fw-bold">
                                {isArabic ? "الطالب" : "Student"}
                              </th>
                              <th className="py-3 border-0 text-secondary text-uppercase small fw-bold">
                          ...
                          <tbody className="border-0">
                            {availableStudents.map((student) => (
                              <tr
                                ...
                                className={selectedStudents.includes(student.id) ? "bg-success-subtle" : ""}
                              >
