                ...
                return (
                  ...
                  >
                    <div className="row g-3">
                      ...
                      {/* Right – video upload / progress / play */}
                      <div className="col-lg-4 d-flex flex-column justify-content-between">
                        <div className="d-flex flex-column gap-2 h-100">

                          {/* ── Uploading state: progress bar + cancel ─────── */}
                          {!isReadOnly && isUploading ? (
                            <div className="bg-white rounded-3 p-3 d-flex flex-column gap-2 flex-grow-1">
                              <div className="d-flex align-items-center justify-content-between mb-1">
                                <span className="small fw-medium text-dark text-truncate" style={{ maxWidth: "140px" }}>
                                  {lesson.video || (isArabic ? "جاري الرفع..." : "Uploading…")}
                                </span>
                                <button
                                  type="button"
                            ...
                            /* ── Normal state: file picker or play button ──── */
                            <label
                              className={`btn ${
                                isComplete || lesson.video
                                  ? "bg-danger text-light"
                                  : "bg-white text-secondary"
                              } border-0 rounded-3 p-3 mb-0 d-flex align-items-center justify-content-center gap-2 flex-grow-1`}
                              style={{
                                cursor:
                                  (isReadOnly && lesson.video) || (!isReadOnly)
                                    ? "pointer"
                                    : "default",
                                minHeight: "54px",
                              }}
                              onClick={() => {
                                if (isReadOnly && lesson.video) {
                                  // Pass raw path – handlePlayVideo in AdminCourses
                                  // will prefix it with the storage base URL
                            ...
                            >
                              ...
                              {/* Hidden file input – only visible in edit mode */}
                              {!isReadOnly && !isUploading && (
                                <input
                                  type="file"
                                  accept="video/*"
                          ...
                          <div className="d-flex align-items-center justify-content-between px-2">
                            <div className="small text-muted d-flex align-items-center gap-1">
                              ...
                              <span>
                                {lesson.durationFormatted || lesson.duration || "0:00"}
                              </span>
                            </div>
                            <div className="d-flex align-items-center gap-1">
                              {/* Preview button in edit mode: use server URL when
                                  upload completed, fall back to blob URL while
                                  still in the same session */}
                              {!isReadOnly && !isUploading && (lesson.uploadedVideoUrl || lesson.blobUrl) && (
                                <button
                                  type="button"
                                  ...
                                  onClick={() =>
                                    handlePlayVideo(
                                      lesson.uploadedVideoUrl || lesson.blobUrl,
                                      lesson.title,
                                    )
                                  }
                                >
                              ...
                              {!isReadOnly && (lesson.video || lesson.uploadedVideoUrl) && !isUploading && (
                                <button
                                  type="button"
