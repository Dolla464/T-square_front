    ...
    return (
        <div className="cta-journey-section text-center text-white py-5 mt-5">
            <Container className="py-5 position-relative z-1">
                ...
                {
                    user ? <Link to={`/${user.role}`} className="btn-cta-enroll text-decoration-none">{isArabic ? "اذهب إلى لوحة التحكم" : "Go to Dashboard"}</Link> : <Link to="/signup" className="btn-cta-enroll text-decoration-none">{isArabic ? "ابدأ رحلتك" : "Start Your Journey"}</Link>
                }
            </Container>
        </div>
    )
